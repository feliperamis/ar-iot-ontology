...
public class Termometro extends Agent {
    ...
    protected void setup() {
        ...
        try {
            ...
            if (result.length>0)
                Termostat = result[0].getName();
        } catch (FIPAException ex) {
            Logger.getLogger(Termometro.class.getName()).log(Level.SEVERE, null, ex);
        }


        SendMessageTickerBehaviour Termometre = new SendMessageTickerBehaviour(this, 10000);
        this.addBehaviour(Termometre);
